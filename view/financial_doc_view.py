...
class FinancialDocView:
    ...
    def __init__(self, user):
        ...
        self.date_time = TextWithLabel(win, "Date", 20, 220, disabled=True)
        ...
        Button(win, text="New", width=8, command=self.reset_form, bg="lightgreen").place(x=35, y=250)
        Button(win, text="Save", width=8, command=self.save_click).place(x=120, y=250)
        Button(win, text="Edit", width=8, command=self.edit_click).place(x=35, y=290)
        Button(win, text="Remove", width=8, command=self.remove_click, bg="red").place(x=120, y=290)
