...
class SimCardDa(Da):
    def save(self, sim_card):
        if (sim_card.owner and self.find_sim_card_count_by_owner_id(sim_card.owner.person_id)< 3):
            self.connect()
            self.cursor.execute("INSERT INTO SIM_CARD_TBL(NUMBER, OPERATOR, PRICE, OWNER_ID) VALUES(%s,%s,%s,%s)",
                                [sim_card.number,
                                 sim_card.operator,
                                 sim_card.price,
                                 sim_card.owner.person_id if sim_card.owner else None])
    ...
    def find_sim_card_count_by_owner_id(self, owner_id):
        self.connect()
